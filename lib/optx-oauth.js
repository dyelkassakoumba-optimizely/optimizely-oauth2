...
OptimizelyOauth.prototype = {
  ...
  _setClientId: function (clientId) {
    this._query.client_id = `client_id=${clientId}`
  },

  _setClientSecret: function (clientSecret) {
    this._query.client_secret = `client_secret=${clientSecret}`
  },
  ...
  _returnGetUrl: function () {
    ...
    return `${this.host}authorize?${this._query.client_id}&${this._query.client_secret}${this._query.redirectUri}${this._query.responseType}${this._query.scope}${this._query.state}${this._query.accountId}`
  },

  _returnPostUrl: function (code) {
    if (!this._query.client_id) throw new Error('The `clientId` parameter has not been set')
    if (!this._query.client_secret) throw new Error('The `clientSecret` parameter has not been set')
    return `${this.host}token?code=${code}&${this._query.client_id}&${this._query.client_secret}${this._query.redirectUri}&grant_type=authorization_code`
  },

  _returnRefreshUrl: function (opts) {
    if (!this._query.client_id) throw new Error('The `clientId` parameter has not been set')
    if (!this._query.client_secret) throw new Error('The `clientSecret` parameter has not been set')
    return `${this.host}token?refresh_token=${opts.refreshToken}&${this._query.client_id}&${this._query.client_secret}&grant_type=refresh_token`
  },
  ...
  _initialize: function (opts) {
    if (!this._query.client_id) this._setClientId(opts.clientId)
    if (!this._query.client_secret) this._setClientSecret(opts.clientSecret)
  ...
  Refresh: function (opts) {
    return new Bluebird((resolve, reject) => {
      if (!opts.refreshToken) throw new Error('The `refreshToken` property must be provided')
      this._initialize(opts)
